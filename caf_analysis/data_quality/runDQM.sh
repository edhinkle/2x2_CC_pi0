...
RUN_NAME="Physics_DQM_Run1_FullRun1NominalHVv11"
INPUTFILELIST="$(pwd)/include/config_data/FullRun1NominalHVv11FileList.txt"
SAMPLE_STRING='Sandboxv11'  # Used for naming output files and directories, e.g. Physics_DQM_Run1_Sandboxv11.root
# FullRun1NominalHVv11FileList.txt
OUTPUTFILE="${RUN_NAME}.root"
MCONLYSTRING="0"
#CONFIGFILEPATH="$(pwd)/include/config_data/PhysicsDQM.yaml"
...
if [[ $RUN_ANALYSIS -eq 1 ]]; then
    ...
    shifter --image=fermilab/fnal-dev-sl7:latest --module=cvmfs,gpu /bin/bash << EOF1
        ...
        # Compile physics DQM code
        echo "Compiling Physics DQM code..."
        source compileDQM.sh

        # Run the analysis
        echo "Running Physics DQM analysis code..."
        ./physicsDQM ${INPUTFILELIST} ${OUTPUTFILE} ${MCONLYSTRING} 
...
if [[ $RUN_PLOTTING -eq 1 ]]; then
    ...
    shifter --image=deeplearnphysics/larcv2:ub2204-cu121-torch251-larndsim bash << EOF2
        ...
        # Install python plotting code in DQM repo
        echo "Installing Python plotting dependencies..."
        pip install -e .

        # Run plotting script
        echo "Plotting histograms from output file ${OUTPUTFILEPATH} with theme ${THEME}..."
        python plotdqmpy/scripts/plot_histograms.py --root_file ${OUTPUTFILEPATH} --sample_name ${SAMPLE_STRING} --theme ${THEME} --all_plots_dir ${OUTPUTDIR} --mcOnly ${MCONLYSTRING}
