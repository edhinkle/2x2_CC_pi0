...
int shower_particle_truth_CC1pi0_no_charged_pions_kaons(const std::string& file_list, bool is_flat = true)
{
    ...
    for(unsigned long i = 0; i < nspills; ++i)
    {
        ...
        for(unsigned long ixn = 0; ixn < num_ixn; ++ixn)
        {
            ...
            auto true_ixn_is_cc = 0;
            auto true_ixn_target = -1;
            ...
            {   
                ...
                true_ixn_is_cc = truth_ixn.iscc;
                true_ixn_target = truth_ixn.targetPDG;
            ...
            if(true_ixn_target != 1000180400)
                continue;
            // Reject NC interactions
            if(true_ixn_is_cc != true)
                continue;
            // Reject interactions without 1 primary pi0
            if(!(true_ixn_pi0s == 1))
                continue;
            if(true_ixn_chkaon > 0 | true_ixn_chpi > 0)
                continue;


            // Require TRUE vertex to be within the TPCs
            if (!(std::isfinite(true_ixn_vtx_x) && std::isfinite(true_ixn_vtx_y) && std::isfinite(true_ixn_vtx_z)))
                continue;
            if (true_ixn_vtx_x < det_x_min || true_ixn_vtx_x > det_x_max)
                continue;
            if (true_ixn_vtx_x > mod23_x_max && true_ixn_vtx_x < mod12_x_min)
                continue;
            if (true_ixn_vtx_y < det_y_min || true_ixn_vtx_y > det_y_max)
                continue;
            if (true_ixn_vtx_z < det_z_min || true_ixn_vtx_z > det_z_max)
                continue;
            if (true_ixn_vtx_z > ups_z_max && true_ixn_vtx_z < downs_z_min)
                continue;
            ...
            // Require at least one TRUE photon showers
            //if(true_ixn_gammas < 1 & true_ixn_electrons < 1)
            //    continue;
...
//
    ...
    std::string file_name = "shower_particle_truth_CC1pi0_no_charged_pions_kaons_sample";
