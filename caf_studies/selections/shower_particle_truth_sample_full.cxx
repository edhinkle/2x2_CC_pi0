...
int shower_particle_truth_sample_full(const std::string& file_list, bool is_flat = true)
{
    ...
    for(unsigned long i = 0; i < nspills; ++i)
    {
        ...
        for(unsigned long ixn = 0; ixn < num_ixn; ++ixn)
        {
            ...
            if(true_ixn_is_cc != true)
                continue;
            // Reject interactions without primary pi0, photons, or electrons
            if(true_ixn_pi0s == 0 & true_ixn_electrons == 0 & true_ixn_gammas == 0)
                continue;
...
//
    ...
    std::string file_name = "shower_particle_truth_FULL_sample";
