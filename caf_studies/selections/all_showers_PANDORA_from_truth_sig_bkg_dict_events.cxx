...
int all_showers_PANDORA_from_truth_sig_bkg_dict_events(const std::string& file_list, const std::string& json_file_path, bool is_flat = true)
{
    ...
    for(unsigned long i = 0; i < nspills; ++i)
    {
        ...
        event_indices.clear();
        if (std::find(event_ids.begin(), event_ids.end(), sr->meta.nd_lar.event) != event_ids.end()) {
...
//
            ...
            } //end of reco particles second for loop
    ...
    std::string file_name = "pi0_all_showers_PANDORA_from_truth_sig_bkg_dict_events";
