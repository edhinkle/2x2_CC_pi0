...
# Read a file and dump it -- keep_all_dets set to True for CC 1pi0 studies
def dump(input_file, output_file, keep_all_dets=True):
    ...
    for jentry in tqdm(range(entries)):
        ...
        # ONLY SAVE IF CC 1PI0 AND NUMU/NUMUBAR
        # ------------------------------------------------
        if genieTree:
        ...
        # REQUIRED FOR SIGNAL: nu_mu(_bar), CC interaction, 1 pi0, no final state mesons
        # HERE, for signal + bkg, REQUIRE: CC interaction, 1+ muon AND (1+ electron or photon OR 1+ pi0 OR 1+ eta) 
        #if abs(nu_pdg) != 14: continue

        count_of_pi0s = part_pdg.count(111)
        count_of_etas = part_pdg.count(221)
        count_of_e = part_pdg.count(11) + part_pdg.count(-11)
        count_of_mu = part_pdg.count(13) + part_pdg.count(-13)
        count_of_photons = part_pdg.count(22)
        
        at_least_one_pi0 = count_of_pi0s >= 1
        at_least_one_muon = count_of_mu >= 1
        at_least_one_shower = (count_of_e + count_of_photons) >= 1
        at_least_one_eta = count_of_etas >= 1

        if at_least_one_muon == False: continue

        enough_showers = at_least_one_shower | at_least_one_pi0 | at_least_one_eta
        if enough_showers == False: continue
